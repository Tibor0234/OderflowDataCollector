...
async def fetch_news(pair: str):
    ...
    last_news_id = 0
    news_file = get_file_path(pair, "news")
...
# --- HISTORY FETCHER ---
# ---------------------------
async def fetch_history(pair: str):
    """Binance klines pollolása és fájlba írás (napi és 15m gyertyák)."""
    
    history_file = get_file_path(pair, "history")
    url = "https://fapi.binance.com/fapi/v1/klines"
    
    async with httpx.AsyncClient() as client:
        try:
            # --- 1. Utolsó 30 napi gyertya ---
            params_daily = {"symbol": pair.upper(), "interval": "1d", "limit": 30}
            resp_daily = await client.get(url, params=params_daily)
            daily_candles = resp_daily.json()

            # formázzuk JSON-compatibilissé
            daily_list = [
                {
                    "open_time": c[0],
                    "open": c[1],
            ...
            daily_record = {"type": "daily", "candles": daily_list}
            ...
            params_15m = {"symbol": pair.upper(), "interval": "15m", "startTime": start_time}
            ...
            intraday_record = {"type": "intraday", "candles": intraday_list}

            # --- Egy fájlba írás, minden batch egy sor ---
            ...
            print(f"{pair} history fetch error:", e)
        ...
        try:
            async with websockets.connect(url) as ws:
                while True:
                    ...
                    bids = [(Decimal(p), Decimal(q)) for p, q in data.get("b", []) if Decimal(p) >= bottom]
...
async def main():
    ...
    for pair in PAIRS:
        tasks.extend([
            ...
            asyncio.create_task(fetch_history(pair))
        ])
